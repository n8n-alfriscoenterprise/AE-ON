...
function renderStaff(){
  ...
  staff.forEach((s,i)=>{
    ...
    const chTag=s.canViewCountHistory?`<span style="font-size:9px;background:#F3E5F5;color:#6A1B9A;padding:1px 6px;border-radius:10px;font-weight:600;margin-right:3px">📊CH</span>`:'';
    ...
    row.innerHTML=`
      <div class="staff-info" style="flex:1;min-width:0">
        ...
        <div class="staff-meta" style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;margin-top:3px">
          ...
          ${saTag}${chTag}${plTag}${distTag}${retailTag}${poTag}${trfTag}${prodTag}${boDistTag}${boRetailTag}
        </div>
      </div>
...
async function addStaff(){
  ...
  const canViewCH  =document.getElementById('new-can-count-history')?document.getElementById('new-can-count-history').checked:false;
